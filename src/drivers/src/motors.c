...
void motorsSetRatio(uint32_t id, uint16_t ithrust) // motor_ID, thrust [PWM]
{
  if (isInit) {
    ...
    ratio = ithrust; // If initializing set motor ratio w/ PWM value

    #ifdef ENABLE_THRUST_BAT_COMPENSATED // WE HAVE THIS TURNED ON
      if (motorMap[id]->drvType == BRUSHED) // Set PWM value w/ battery compensation
      {
        float thrust = ((float)ithrust / 65536.0f) * 65; // Convert desired PWM to g but why 60g if each motor maxes out at 15g
        float volts = -0.0006239f * thrust * thrust + 0.088f * thrust; // Map thrust [g] to [V]
        float supply_voltage = pmGetBatteryVoltage();
        float percentage = volts / supply_voltage;
        percentage = percentage > 1.0f ? 1.0f : percentage; // If % > 100%, then cap at 100% else keep same
        ratio = percentage * UINT16_MAX; // remap percentage back to PWM range
